...
const groupMetrics = (metrics) => {
  return Object.entries(metrics).reduce((state, [name, value]) => {
    ...
    const [appId, goblin, metric] = ns;
    const cleanRegex = /[^a-zA-Z_:][^a-zA-Z0-9_:]*/g;
    const sanitized = [
      appId.replace(cleanRegex, ''),
      goblin.replace(cleanRegex, ''),
      metric.replace(cleanRegex, ''),
    ];
    state.push(`${sanitized.join('_')}_total ${value}\n`);
    return state;
  }, []);
};
